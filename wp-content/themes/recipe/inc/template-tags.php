...
if ( ! function_exists( 'recipe_posted_on' ) ) :
	...
	function recipe_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'recipe' ),
...
if ( ! function_exists( 'recipe_entry_footer' ) ) :
	...
	function recipe_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'recipe' ) );
			if ( $tags_list ) {
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'recipe' ),
						array(
							'span' => array(
								'class' => array(),
							),
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'recipe' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if ( ! function_exists( 'recipe_post_thumbnail' ) ) :
	/**
	 ...
	 */
	function recipe_post_thumbnail() {
